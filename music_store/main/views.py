...
class BaseView(CartMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        categories = Category.objects.get_category_counts(**kwargs)
        products = LatestProducts.objects.get_products_for_main_page('speakersproduct', 'earphonesproduct',
                                                                     with_respect_to='speakersproduct')
        context = {
...
class ChangeQTYView(CartMixin, View):
    def post(self, request, *args, **kwargs):
        ...
        cart_product.save()
        recalc_cart(self.cart)
        messages.add_message(request, messages.INFO, "Количесвто товаров изменено")
        return HttpResponseRedirect('/cart/')
